...
| <a name="input_cloud_services"></a> [cloud\_services](#input\_cloud\_services) | The list of cloud services used to create endpoint gateways. If `vpe_name` is not specified in the list, VPE names are created in the format `<prefix>-<vpc_name>-<service_name>`. The value that you specify for `vpc_name` must be known at Terraform plan time. Set `existing_vpe_id` to use a pre-existing gateway instead of creating a new one; the gateway itself will not be destroyed on `terraform destroy`. | <pre>set(object({<br/>    service_name                = string<br/>    vpe_name                    = optional(string), # Full control on the VPE name. If not specified, the VPE name will be computed based on prefix, vpc name and service name.<br/>    existing_vpe_id             = optional(string)  # ID of a pre-existing VPE Gateway. When set, no new gateway is created; only reserved IPs and gateway-IP bindings are managed.<br/>    dns_resolution_binding_mode = optional(string, "disabled")<br/>  }))</pre> | `[]` | no |
