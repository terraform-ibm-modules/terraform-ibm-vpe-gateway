...
output "crn" {
  ...
  value       = [for vpe_crn in data.ibm_is_virtual_endpoint_gateway.vpe : vpe_crn.crn]
}

output "vpe_ids" {
  description = "A map of endpoint gateway names to their IDs"
  value       = { for name, gateway in data.ibm_is_virtual_endpoint_gateway.vpe : name => gateway.id }
}
